          ...
          {!isUser && message.rules_cited && message.rules_cited.length > 0 && (
            <div className="mt-3 pt-2.5 border-t border-slate-100 flex flex-wrap items-center gap-1.5">
              ...
              {message.rules_cited.map(rule => (
                <span
                  key={rule}
                  className="text-[10px] font-bold bg-slate-100 border border-slate-200 text-slate-700 px-2 py-0.5 rounded-md hover:bg-slate-200 transition-colors cursor-default"
                >
                  {rule}
                </span>
              ))}
